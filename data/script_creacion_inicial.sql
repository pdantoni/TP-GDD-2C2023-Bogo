...
CREATE FUNCTION BOGO.OBTENER_ID_SUCURSAL(@nombre Varchar(250)) RETURNS INT AS 
BEGIN 
    DECLARE @id_sucursal INT 
    SELECT @id_sucursal = s.codigo_sucursal from Bogo.Sucursal s where s.nombre = @nombre
    RETURN @id_sucursal
END
GO

CREATE FUNCTION BOGO.OBTENER_ID_ALQUILER(@nombre INT) RETURNS INT AS 
BEGIN 
    DECLARE @id_alquiler INT 
    SELECT @id_alquiler = a.codigo_alquiler from Bogo.Alquiler a where a.codigo_alquiler = @nombre
    RETURN @id_alquiler
END
GO
...
BEGIN
	INSERT INTO BOGO.Agente_inmobiliario (nombre, apellido, dni, telefono, fecha_nacimiento, mail, fecha_registro, sucursal)
		SELECT DISTINCT	AGENTE_NOMBRE, AGENTE_APELLIDO, AGENTE_DNI, AGENTE_TELEFONO, AGENTE_FECHA_NAC, AGENTE_MAIL, AGENTE_FECHA_REGISTRO, BOGO.OBTENER_ID_SUCURSAL(SUCURSAL_NOMBRE)
...
BEGIN
	INSERT INTO BOGO.Periodo (numero_periodo_de_inicio, numero_periodo_de_fin, precio, alquiler)
		SELECT DISTINCT	DETALLE_ALQ_NRO_PERIODO_INI, DETALLE_ALQ_NRO_PERIODO_FIN, DETALLE_ALQ_PRECIO, BOGO.OBTENER_ID_ALQUILER(ALQUILER_CODIGO) FROM gd_esquema.Maestra 
